...
public class HealthControllerTests
{
    ...
    {
        var mockHealthCheckService = new Mock<HealthCheck.IHealthCheckService>(MockBehavior.Strict);
        mockHealthCheckService
            .Setup(
                expression => expression.CheckHealth(default)
            )
            .ReturnsAsync(
                new HealthCheck.HealthReport(
                    [],
        ...
        Assert.IsInstanceOfType<ViewModel.ActionResponse<HealthCheck.HealthReport>>(castedResponse.Value);

        var actionResponse = castedResponse.Value as ViewModel.ActionResponse<HealthCheck.HealthReport>;
